...
class C {				
	...
	static int b = 20;		// 정적 필드 : 객체화 하지 않고 클래스 이름으로 사용가능, 객체화 해도 사용가능
									// 모든 객체에서 공유되는 변수
								// 메모리의 클래스 영역에 변수의 값을 저장
								// Heap 영역에는 클래스 영역의 저장된 주소값만 저장
	
	void print1() {
		System.out.println(a + " " + b);
	}
	static void print2() {		// 정적 메소드에는 인스턴스 필드가 올 수 없다. a : 인스턴스 필드
								// 정적 메소드에서는 정적 필드만 올 수 있다. b : 정적 필드
...
public class Instance_VS_Static02 {
	...
	public static void main(String[] args) {
		...
		c1.a = 888;
		c2.a = 999;
		c3.a = 777;		// a : 그 객체에만 적용되는 고유한 값 
		c1.b = 111;
		C.b = 333;		// 모든 객체에서 공유도ㅚ는 블락
