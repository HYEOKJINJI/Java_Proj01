...
class Animal{
	...
	void eat() {
		System.out.println("모둔 동물은 먹습니다.");
	}
}
...
public class TypeCasting01 {
	public static void main(String[] args) {
		...
		// 1. 업캐스팅 : Tiger(자식 타입) 객체 ==> Animal(부모 타입)
		// 업캐스팅 : 자식 객체를 생성해서 부모 타입으로 정의, 자동으로 일어남 
		// a1 : Tiger 의 필드와, 메소드 + Animal 의 필드와, 메소드
		//	Animal 의 필드와, 메소드만 사용 가능
		...
		// 3. Tiger 타입으로 객체 생성 : Animal, Tiger 의 필드와 메소드 모두 접근
		// t3 : Animal, Tiger 모두 내포
		Tiger t3 = new Tiger();
		
		// 다운캐스팅 : 실행시 오류 발생, t3 에는 TigerChild 를 내포하지 않아서 다운캐스팅 실행시 오류발생
		...
		// 다운캐스팅 할때에는 반드시 instanceof 를 사용해야 한다.
		...
		// 4. a3 를 TigerChild 로 다운캐스팅
		TigerChild t6 = null;		// 블락 밖에서 변수 선언
		if (t3 instanceof TigerChild) {
			t6 = (TigerChild) t3;
		}
		// t6 : TigerChild 타입(Animal, Tiger, TigerChild 의 필드와 메소드 모두 접근)
		t6.age = 2;		// Animal
		t6.tigerSound = "어흥 어훙";	// Tiger
