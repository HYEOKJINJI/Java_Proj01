...
class A {		// Outer Class
	...
	static class B{		// static 이너 클래스 : static 필드, static 메소드만 포함 할 수 있다.
		void bcd() {
			// 1. 외부 클래스의 인스턴스 필드, 메소드 : 접근 불가
//			System.out.println(a);	// 인스턴스 필드 이여서 오류 발생 : 접근 불가
			System.out.println(b);	// 정적(static) 필드 : 접근 가능
			
//			method1();		// 인스턴스 메소드 이여서 오류 발생 : 접근 불가
			method2();		// 정적(static) 메소드 : 접근 가능
		}
