...
public class Initial_Value {
	...
	public static void main(String[] args) {
		...
		//	참조 자료형 (배열) : Stack 공간에 [배열] 변수와 Heap 영역의 주소 정보가 저장됨
		//					Heap 영역에는 값이 저장됨
				// 강제 초기화가 됨. boolean(false), 정수(0), 실수(0.0)
						//		char(공백), String(null)
		
		// 1. Stack 메모리 값(강제 초기화 안됨)
		int value1;
		// System.out.println(value1);	// 오류 발생 : 초기값이 없다.
		int[] value2;
		// System.out.println(value2);	// 오류 발생 : value2 에 Heap 영역에 주소값이 없다.
		...
		int[] value4 = null;		// null : 비어있는 상태, 기본자료형에는 사용 할 수 없다. 
									//참조 자료형에만 사용 가능
		System.out.println(value4);	// value4 에 Heap 영역의 주소가 비어있는 상태 : null
		
		System.out.println("=======boolean : 초기값으로 false===========================================================================================");
		
		// 2. Heap 영역은 강제 초기화가 된다. boolean(false), 정수(0), 실수(0.0)
		// 2-1. 기본 자료형 배열
		
		boolean[] arr1 = new boolean[3];	// 기본값으로 false 가 각 방에 할당됨
