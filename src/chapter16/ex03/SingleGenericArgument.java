...
//제너릭 클래스 : 다양한 타입을 저장 할 수 있는 클래스
	// 클래스 이름 뒤 <T>
	// 객체를 생성 할 때 <T> 들어오는 타입을 지정해서 객체를 생성
	// <T> 들어오는 타입은 객체형 타입만 들어와야 한다. wrapper class 를 사용 해야한다.
...
public class SingleGenericArgument {
	...
	public static void main(String[] args) {
		...
		// Apple 객체를 setter 를 사용해서 주입 하고 값 가져오기
		app1.set(new Apple());
		System.out.println(app1.get().apple);
		
		// Pencil 객체를 setter 를 사용해서 주입 하고 값 가져오기
		MyClass<Pencil> pen1 = new MyClass<Pencil>();
		pen1.set(new Pencil());					// setter 를 사용해서 Pencil 객체를 주입
		System.out.println(pen1.get().pencil);	// getter 를 사용해서 Pencil 객체를 가져옴
		
		// Banana 객체를 setter 를 사용해서 주입 하고 값 가져오기
