...
public class If_Controll_Startment {

	public static void main(String[] args) {
		// IF 구문 사용하기
		// 1. type 1	: if (조건) {실행문 : 조건이 참일때 실행}
			// {} 를 생략 가능한데, 생략될 경우에는 실행문 하나만 적용
		int value1 = 5;
		if (value1>3) {
			// 조건이 참일때 실행블락
		...
		//조건이 거짓일때
		if (value1 < 3) {
			System.out.println("실행2");		// 조건이 거짓이므로 실행이안됨
		}
		System.out.println("====={조건이 참일때 실행문}, {} 생략가능=================");
	
			//if 조건에서 실행문이 하나일때 {} 생략이 가능
		if (value1<3) 
			System.out.println("실행3");		// 실행문 1개만 {} 가 생략되서 처리될 수 있다.
		...
		// if (조건1) {조건1이 참일때 실행} else if (조건2) {조건2가 참일때 실행} ...else {조건에 존재하지 않은경우 실행}
		int value3 = 85;
