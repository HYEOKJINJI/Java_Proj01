...
interface B {
	...
	// public abstract 생략 : 모든 메소드는 선언부만 있고 구혀분가 없어야 한다. (예외 : default, static 메소드)
	void abc();
}
...
// 추상 클래스 : 추상 메소드를 포함하고 있는 클래스, 추상 클래스내의 추상 메소드는 abstract 를 생략 하면 안됨.
abstract class C {
...
class Cc extends C{
	@Override
	void abc() {
		System.out.println("C 추상 클래스를 구현한 메소드 : Cc 클래스의 오버라이딩된 메소드");	
	}
}
		...
		/*
		 	인터페이스(Interface) : 내부와 외부를 연결 시켜주는 접점, 콘센트 
		 		...
		 		- 모든 필드는 public static final 이 생략됨
		 			: 값을 수정 할 수 없다.
		 		- 모든 메소드는 public abstract 가 생략됨 (예외 : default, static 메소드 : 1.8 버젼)
		 			: 모든 메소드는 선언부만 있어야 한다. 구현부가 있으면 안됨
		 			: 예외 : default, static 메소드는 인터페이스에서 유일하게 구현부를 갖는 메소드
...
//		B b = new B ();
		...
		// 4. 필드의 값을 수정 할 수 없다. 
//		A.a = 10;    오류 발생 final 로 인하여 값 수정 불가
//		B.a = 20;	 오류 발생 final 로 인하여 값 수정 불가
		
		System.out.println("==== 추상 클래스 (C) =================================");
		
		// 1. 추상 클래스 : 객체 생성 불가.
//		C c1 = new C();		C : 추상 클래스 이므로 객체생성이 안됨
		
		// 2. 추상 클래스 : 구현한 클래스를 객체화 해서 타입 지정은 가능
		C c2 = new Cc();
		System.out.println(c2.m);	// 인스턴스 필드 : 객체 생성 후 호출 가능
//		System.out.println(C.m);	// 인서턴스 필드 : static 이 없으므로 오류 발생
		
		c2.m = 44;
		System.out.println(c2.m);	// 44 : 값 수정이 가능
		
		c2.bcd();		// C 클래스의 일반 메소드(인스턴스 메소드)
		c2.abc();		// 오버라이딩된 메소드 출력, C 타입의 abc() 를 호출, 출력 Cc 타입의 abc()
