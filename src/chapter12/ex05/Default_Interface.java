...
// 가정 : interface A 를 구현한 클래스가 1000 개 존재한다.
...
public class Default_Interface {
	...
	public static void main(String[] args) {
		// interface : 인터페이스의 모든 필드와 메소드는 public이 생략 되어잇다.
				// 구현한 클래스에서 메소드를 오버라이딩 할 때 public 
		// 모든 필드는 : public static final 
		// 모든 메소드는 추상 메소드 이여야 한다. public abstract
			// 예외 : 1.8 버젼 부터 새로운 기능이 추가, default, static 메소드가 추가됨
				// 인터페이스에서 구현부가 있는 메소드. default, static
		// default : 인터페이스에서 구현부가 있는 메소드
					// 인터페이스를 구현한 모든 클래스에 상속되어 적용.
					// 오버라이딩도 가능
					// 새로운 기능을 추가 할 때 하위의 구현된 클래스의 오류 없이 적용 가능 하도록 함.
		
		// 1. 인터페이스를 구현한 클래스를 객체화해서 타입으로 지정.
		A a1 = new Aa ();
		A a2 = new Aaa();
		A a3 = new Aaaa();
