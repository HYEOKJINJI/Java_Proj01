...
abstract class Animal {		// 클래스 내부에 추상 메소드가 1개라도 존재하면, 추상 클래스가 되어야 한다.
	...
	void eat () {
		...
		// 추상 메소드 : 실행 블락이 존재하지 않는 메소드
			...
			// 프로잭트 설계시 메소드 이름을 다른 이름으로 사용하지 못하도록 할 수 있다.
			// 부모 클래스에서는 메소드 이름을 선언만 해주고, 자식 클래스에서 구현 하도록 함.
...
class Cat extends Animal{	// 콘크리트 클래스 : 부모의 추상 메소드를 모두 구현한 클래스
	// 부모 클래스에서 추상 메소드가 존재 할 경우 자식에서는 추상 메소드를 오버라이딩해서 구현 해야 한다.
